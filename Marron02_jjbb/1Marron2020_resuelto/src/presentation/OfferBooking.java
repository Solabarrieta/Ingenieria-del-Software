...
public class OfferBooking extends JFrame {
	...
	//nuevo combo
	private JComboBox <Offer> newCombo;
	private DefaultComboBoxModel <Offer> offerInfoBox = new DefaultComboBoxModel <Offer> ();
	...
	public OfferBooking() {
		...
		lookforOffers.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				bookOffer.setEnabled(true);
				offerInfoBox.removeAllElements(); // limpia
				bookOffer.setText("");
				...
				try {
					...
					searchResult.setText("Error en fecha");
				}
			}
		});

		lookforOffers.setBounds(94, 124, 261, 40);
		contentPane.add(lookforOffers);	

		searchResult = new JLabel("");
		searchResult.setBounds(112, 175, 217, 16);
		contentPane.add(searchResult);


		//1) Nuevo ComboBox 
		newCombo = new JComboBox();
		newCombo.setBounds(99, 202, 243, 26);
		contentPane.add(newCombo);
		newCombo.setModel(offerInfoBox);
		newCombo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {

				if (newCombo.getItemCount() != 0) {
					//itemCombo = newCombo.getItemAt(i);
					itemCombo = (Offer) newCombo.getSelectedItem();
					...
					if (tripleRooms.isSelected()) { 
						num = businessLogic.cantdHabitaciones(itemCombo, 3);
						//num = itemCombo.getTripleNumber();
						if(num == 0) {
							error=true; 
						}
					} else if (doubleRooms.isSelected()) {
						num = businessLogic.cantdHabitaciones(itemCombo, 2);
						//num = itemCombo.getDoubleNumber();
						if (num == 0) {
							error=true;
						}
					} else if (singleRooms.isSelected()) {
						num = businessLogic.cantdHabitaciones(itemCombo, 1);
						//num = itemCombo.getSingleNumber();
						if (num == 0) {
							error=true;
						}
					}
					...
					if (error) {
						...
						bookOffer.setEnabled(true);
						bookOffer.setText("Book this offer: " + itemCombo);
						businessLogic.storeOff(itemCombo);
					}		
				}
			}
		});
		...
		offerList.setBounds(64, 263, 336, 71);
		...
		bookOffer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (tripleRooms.isSelected()) { 
					num=itemCombo.getTripleNumber();
					if (num>0) itemCombo.setTripleNumber(num-1); else error=true; 
				}
				else if (doubleRooms.isSelected()) {
					num=itemCombo.getDoubleNumber();
					if (num>0) itemCombo.setDoubleNumber(num-1); else error=true;
				}
				else if (singleRooms.isSelected()) {
					num=itemCombo.getSingleNumber();
					if (num>0) itemCombo.setSingleNumber(num-1); else error=true;
				}
		...
		bookOffer.setBounds(51, 258, 330, 40);
